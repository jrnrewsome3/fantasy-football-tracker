...
/** Known acceptable errors when DB/data is unavailable in CI/local test envs. */
function isExpectedUnavailableError(error: unknown): boolean {
  const message = error instanceof Error ? error.message : String(error);
  return (
    message.includes("No matchups found") ||
...
describe("generateWeeklyRecap", () => {
  it("should deduplicate top performers by team name", async () => {
    ...
    //
    // Note: This test will skip assertions if no matchup/DB data exists for the league/week.
    ...
    try {
      ...
      if (isExpectedUnavailableError(error)) return;
      throw error;
    }
  });
