...
import { ArrowLeft, Calendar, Users, Trophy, TrendingUp } from "lucide-react";
...
export default function BrowseSeasons() {
  ...
  const { data: seasonSummaries, isLoading: summariesLoading } = trpc.league.seasonSummaries.useQuery(
    { espnLeagueId },
    { enabled: !!user && !!espnLeagueId }
  );

  // Get league name from first available league with this ESPN ID
  const leagueName = leagues?.find(l => l.espnLeagueId === espnLeagueId)?.name || "League";
    ...
    <div className="min-h-screen bg-background">
      <div className="container py-8">
        ...
        <div className="space-y-6">
          {/* Header */}
          <div>
            <h1 className="text-3xl font-bold text-card-foreground">Browse Seasons</h1>
          ...
          {/* Season Cards Grid */}
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {seasonSummaries.map(({ league, teamCount, totalGames, topScorer }) => (
              <Card 
                key={league.id} 
              ...
              >
                <CardHeader className="border-b bg-gradient-to-br from-primary/5 to-transparent">
                  <div className="flex items-center justify-between">
                    <div>
                      <CardTitle className="text-2xl">{league.seasonYear}</CardTitle>
                ...
                <CardContent className="pt-6 space-y-4">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className="font-semibold text-card-foreground">{teamCount}</span>
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className="font-semibold text-card-foreground">{totalGames}</span>
                  </div>
                  ...
                  {topScorer && (
                    <div className="pt-2 border-t">
                      ...
                      <div className="ml-6">
                        <p className="font-semibold text-card-foreground text-sm">{topScorer.name}</p>
                        <p className="text-xs text-muted-foreground">{topScorer.points.toLocaleString()} points</p>
                      </div>
                    </div>
                  )}
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  ...
                  <p className="text-2xl font-bold text-card-foreground">{seasonSummaries.length}</p>
                </div>
                <div>
                  <p className="text-sm text-muted-foreground">Years Covered</p>
                  <p className="text-2xl font-bold text-card-foreground">
                    {seasonSummaries[seasonSummaries.length - 1]?.league.seasonYear} - {seasonSummaries[0]?.league.seasonYear}
                  </p>
                </div>
                <div>
                  <p className="text-sm text-muted-foreground">Total Games Played</p>
                  <p className="text-2xl font-bold text-card-foreground">
                    {seasonSummaries.reduce((sum, s) => sum + s.totalGames, 0).toLocaleString()}
                  </p>
                </div>
              </div>
            </CardContent>
